...
*******************************************************************************/
/**
 * \defgroup group_uart UART (UART STL module)
 * \{
 *
 ...
 * After 256 function calls, when the test finishes testing all 256 values and they
 * are all a success, the function returns \ref PASS_COMPLETE_STATUS (3). All 256
 * bytes are tested so that all cells in the 256-byte FIFO are used.
 *
 *
...
*  \ref ERROR_TX_NOT_EMPTY (4) - TX buffer is not empty <br>
*  \ref ERROR_RX_NOT_EMPTY (5) - RX buffer is not empty <br>
*  \ref ERROR_UART_NOT_ENABLE (6) - UART block is not enabled <br>
*
* \note
...
*  UART instance. The test clears RX/TX FIFOs and temporarily disables UART
*  RX/TX interrupt masks while each byte is checked.
*******************************************************************************/
